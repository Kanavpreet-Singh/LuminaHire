...
/**
 * Approve the reviewed research and kick off evaluation. Sets status
 * EVALUATING (a running status the frontend already polls) and dispatches the
 * background evaluator; it pauses again at AWAITING_EVALUATION_APPROVAL.
 */
export async function POST(
    ...
    try {
        ...
        const pythonResponse = await fetch(`${PYTHON_API_URL}/vet/research/approve-async`, {
            method: "POST",
        ...
        if (!pythonResponse.ok) {
            ...
            throw new Error(errText || "Failed to start evaluation");
        }
